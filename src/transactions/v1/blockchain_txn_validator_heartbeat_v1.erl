...
absorb(Txn, Chain) ->
    ...
    POCMgr = application:get_env(blockchain, poc_mgr_mod, miner_poc_mgr),
        ...
        {ok, V} ->
            ...
            case lists:member(Validator, ConsensusAddrs) of
                true ->
                    ok;
                false ->
                    %% TODO: maybe need to move the current miner_poc_mgr to core
                    POCMgr:save_poc_key_proposals(Validator, POCKeyProposals, TxnHeight)
            end,
